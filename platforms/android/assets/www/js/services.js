    ...
    get: function (force, callback_service) {
      ...
      async.series([
        ...
        function (callback) {
          ...
          $http.get(url)
            .success(function (data, status, headers, config) {
              ...
              traffic_json.data_db._id = data.TimestampAggiornamentoTraffico;
              console.log('_id:' + traffic_json.data_db._id);
              callback(false, 'next');
        ...
        function (callback) {
          ...
          if (!force) {
            _getdoc(pdb, db, traffic_json.data_db._id, function (isdbexist, data) {
              isDb = isdbexist;
              if (!isdbexist) {
        ...
        function (callback) {
          ...
          if (!isDb) {
            ...
            $http.get(url)
              .success(function (data, status, headers, config) {
                  //handle success
                  traffic_json.data_db.traffic = data;
                  console.log('Success.');
        ...
        function (callback) {
          ...
          if (!isDb) {
              ...
              $http.get(url)
                  ...
                  traffic_json.data_db.parking = data;
                  callback(false, 'next');
                }).error(function (data, status, headers, config) {
        ...
        function (callback) {
          ...
          if (!isDb) {
            _save(pdb, db, traffic_json.data_db, callback)
          } else {
            callback(false, 'done');
          };

        }], function (err, results) {
          callback_service(err, traffic_json.data_db);
        });
    }
  };
